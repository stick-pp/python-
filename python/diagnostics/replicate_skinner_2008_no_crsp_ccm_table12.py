...
FORMAL_TABLE_DIR = SHARED_PYTHON_OUT / "tables"
...
def main() -> None:
    """生成无 CRSP/CCM 普通股筛选口径下的 Table 1/2 对比输出。"""
    ensure_dirs()

    main_df, ccm_link, crsp_names = read_inputs()
    clean = clean_and_construct(main_df, ccm_link, crsp_names, use_crsp_ccm_screen=False)
    ...
    for start, end in table1:
        no_screen_total = add_sum_margins(table1[(start, end)]["count_raw"]).loc["Sum", "Sum"]
        formal_path = FORMAL_TABLE_DIR / f"table1_counts_{start}_{end}.csv"
        current_total = add_sum_margins(pd.read_csv(formal_path, index_col=0)).loc["Sum", "Sum"]
        rows.append(
            {
                "table": "Table 1",
                "panel_or_group": f"{start}-{end}",
                "current_crsp_ccm": current_total,
                "no_crsp_ccm": no_screen_total,
                "difference": no_screen_total - current_total,
                "pct_difference_vs_current": (
                    (no_screen_total - current_total) / current_total if current_total else np.nan
                ),
            }
        )
    ...
    group_labels = {
        ...
        2: "Group II: Regular div. + regular rep.",
        3: "Group III: Occasional rep. only",
        4: "Group IV: Regular rep. only",
        5: "Group V: Dividend-only regular",
        0: "Other",
    }
    no_screen_groups = (
        firm_groups[firm_groups["main_sample_entry"]]["group_id"]
        .value_counts(dropna=False)
        .rename("no_crsp_ccm")
    )

    main_clean = clean_and_construct(main_df, ccm_link, crsp_names, use_crsp_ccm_screen=True)
    _, main_firm_groups = add_long_run_groups(main_clean)
    current_groups = (
        main_firm_groups[main_firm_groups["main_sample_entry"]]["group_id"]
        .value_counts(dropna=False)
        .rename("current_crsp_ccm")
    )
    group_compare = (
        pd.concat([current_groups, no_screen_groups], axis=1)
        .fillna(0)
        .astype(int)
        .sort_index()
    )
    for group_id, row in group_compare.iterrows():
        current_total = int(row["current_crsp_ccm"])
        no_screen_total = int(row["no_crsp_ccm"])
        rows.append(
            {
                "table": "Table 2",
                "panel_or_group": group_labels.get(int(group_id), str(group_id)),
                "current_crsp_ccm": current_total,
                "no_crsp_ccm": no_screen_total,
                "difference": no_screen_total - current_total,
                "pct_difference_vs_current": (
    ...
    rows.append(
        {
            "table": "Table 2",
            "panel_or_group": "Total main-sample firms",
            "current_crsp_ccm": int(current_groups.sum()),
            "no_crsp_ccm": int(no_screen_groups.sum()),
            "difference": int(no_screen_groups.sum() - current_groups.sum()),
            "pct_difference_vs_current": (
                (no_screen_groups.sum() - current_groups.sum()) / current_groups.sum()
                if current_groups.sum()
                else np.nan
            ),
        }
    )
    ...
    summary.to_csv(TABLE_DIR / "table12_no_crsp_ccm_vs_current_summary.csv", index=False)
    print(f"Created no-CRSP/CCM Table 1/2 outputs under {TABLE_DIR}")
